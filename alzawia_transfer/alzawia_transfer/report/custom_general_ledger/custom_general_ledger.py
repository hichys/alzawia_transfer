...
def get_gl_entries(filters, accounting_dimensions):
    ...
    for gl in gl_entries:
        # Find related Journal Entry (original or reversal)
        je_name = gl.voucher_no
        reversed_from = frappe.db.get_value("Journal Entry", je_name, "reversal_of")
        if reversed_from:
            je_name = reversed_from  # use the original JE if this one is a reversal

        # Find linked Alzawia Transfer using the Journal Entry
        alzawia_transfer = frappe.db.get_value(
            "Alzawia Transfer",
            {"journal_entry": je_name},  # replace with your actual fieldname
            "name",
        )
        gl["alzawia_transfer"] = alzawia_transfer
